...
class ArrayProperty(PropertyTrait):
    ...
    @classmethod
    def new(cls, property_type: str, field_name: Optional[str] = None,
            type_name: Optional[str] = None, guid: Optional[Guid] = None) -> 'ArrayProperty':
        """Create a new array property"""
        return cls(property_type=property_type, field_name=field_name,
                  type_name=type_name, guid=guid or Guid())
        ...
        if include_header:
            ...
            if self.property_type == "StructProperty":
                ...
                if prop_type != self.property_type:
                    raise DeserializeError(f"Property type mismatch: {prop_type} != {self.property_type}")
                ...
                terminator = stream.read(1)[0]
                if terminator != 0:
                    raise DeserializeError.invalid_terminator(terminator, stream.tell() - 1)
                    
            # Create options for nested properties
            nested_options = PropertyOptions(
                hints=options.hints if options else None,
                property_path=f"{options.property_path}.ArrayProperty" if options else "ArrayProperty"
            )
            ...
            for _ in range(count):
                prop = Property.new(stream, self.property_type, options=nested_options)
            ...
            if actual_size != property_length_to_check:
                raise DeserializeError.invalid_value_size(property_length_to_check, actual_size, start)
        else:
            raise DeserializeError.invalid_property("ArrayProperty is not supported in arrays", stream)
            
    def write(self, stream: BinaryIO, include_header: bool = True,
              options: Optional[PropertyOptions] = None) -> int:
        """Write array to stream"""
        if not include_header:
            raise SerializeError.invalid_property("ArrayProperty is not supported in arrays")
