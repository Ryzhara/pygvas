...
class ArrayProperty(PropertyTrait):
    ...
    def read_body(self, stream: BinaryIO) -> None:
        ...
        #     pass
        elif self.property_type in ["StrProperty", "NameProperty", "ObjectProperty", "EnumProperty"]:
            for _ in range(property_count):
                    string_element = read_string(stream)
                    self.values.append(string_element)

        elif self.property_type == "IntProperty":
            for _ in range(property_count):
                    int_element = struct.unpack("<i", stream.read(4))[0]
                    self.values.append(int_element)
    ...
    def write(
        ...
        if self.property_type == "StructProperty":
            ...
            for int_value in self.values:
                array_bytes += array_buffer.write(struct.pack("<i", int_value))
