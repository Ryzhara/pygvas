...
class SetProperty(PropertyTrait):
    ...
    @classmethod
    def new(cls, property_type: str, allocation_flags: int = 0, properties: Optional[List[Property]] = None) -> 'SetProperty':
        """Create a new set property"""
        return cls(property_type=property_type, allocation_flags=allocation_flags, properties=properties or [])
    
    def read(self, stream: BinaryIO, include_header: bool = True,
             options: Optional[PropertyOptions] = None) -> None:
        """Read set from stream"""
        if not include_header:
            raise DeserializeError.invalid_property("SetProperty is not supported in arrays", stream.tell())
    ...
    def write(self, stream: BinaryIO, include_header: bool = True,
        ...
        if include_header:
            ...
            for prop in self.properties:
                buffer_bytes += prop.write(buffer, include_header=False, options=options)
            ...
            # Write properties
            for prop in self.properties:
                bytes_written += prop.write(stream, include_header=False, options=options)
