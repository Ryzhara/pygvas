        ...
        def read(
            ...
            if size == 1:
                self.value = (
                    ...
                    if signed
                    else struct.unpack("B", stream.read(1))[0]
                )
            elif size == 2:
                self.value = (
                    struct.unpack("<h", stream.read(2))[0]
                    if signed
                    else struct.unpack("<H", stream.read(2))[0]
                )
            elif size == 4:
                self.value = (
                    struct.unpack("<i", stream.read(4))[0]
                    if signed
                    else struct.unpack("<I", stream.read(4))[0]
                )
            else:  # size == 8
                self.value = (
                    struct.unpack("<q", stream.read(8))[0]
                    if signed
                    else struct.unpack("<Q", stream.read(8))[0]
                )
        ...
        def write(
            ...
            if size == 1:
                stream.write(
                    ...
                    if signed
                    else struct.pack("B", self.value)
                )
            elif size == 2:
                stream.write(
                    struct.pack("<h", self.value)
                    if signed
                    else struct.pack("<H", self.value)
                )
            elif size == 4:
                stream.write(
                    struct.pack("<i", self.value)
                    if signed
                    else struct.pack("<I", self.value)
                )
            else:  # size == 8
                stream.write(
                    struct.pack("<q", self.value)
                    if signed
                    else struct.pack("<Q", self.value)
                )
            bytes_written += size
