...
class MapProperty(PropertyTrait):
    ...
    def read(self, stream: BinaryIO, options: Optional[PropertyOptions] = None) -> None:
        ...
        nested_options = PropertyOptions(
            ...
            property_path=(
                f"{options.property_path}.MapProperty" if options else "MapProperty"
            ),
        )
        ...
        for _ in range(num_entries):
            # Read key
            key_prop = Property.new(stream, self.key_type, options=nested_options)

            # Read value
            value_prop = Property.new(stream, self.value_type, options=nested_options)
